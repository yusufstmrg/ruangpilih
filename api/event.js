...
export default async function handler(req, res) {
  ...
  const event = {
    ...
    product_id: body.product_id ? String(body.product_id).slice(0, 160) : null,
  ...
  try {
    await sql()`insert into rp_events(event_name, session_id, product_id, metadata, occurred_at) values (${event.event_name}, ${event.session_id}, null, ${JSON.stringify(event.metadata)}, ${event.occurred_at})`;
